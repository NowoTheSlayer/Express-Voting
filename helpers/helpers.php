<?php
	...
	function loginVoter($voter_id){
		$_SESSION['Voter'] = $voter_id;
		global $db;
		// date_default_timezone_set("Africa/Lagos");
		// $date = date("Y-m-d h:i:s");
		// $db->query("UPDATE users SET last_login = '$date' WHERE id = '$voter_id'");
	...
	function loginAdmin($admin_id){
		...
		// date_default_timezone_set("Africa/Lagos");
		// $date = date("Y-m-d h:i:s");
		// $db->query("UPDATE users SET last_login = '$date' WHERE id = '$admin_id'");

		$_SESSION['success_flash'] = 'You are now logged in!';
		header('Location: index.php');
	}

	function isadmin_logged_in(){
	...
	function isvoter_logged_in(){
		if (isset($_SESSION['User']) && $_SESSION['Voter'] > 0) {
			return true;
		}
		return false;
	}
